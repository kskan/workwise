...
$group = $db->select()->from('group')->where("id = $id")->find();

if(!$group)
{
	showMsg("该角色不存在",config('website').'/admin/grouplist.php');
	exit;
}
...
$currentRule = explode(",",$group['rules']);
...
if($_POST)
{
	...
	$affectrow = $db->update("group",$data,"id = $id");

	if($affectrow)
	{
		unset($_SESSION['adminmenu']);
		adminMenu();
		showMsg('编辑角色成功',config('website').'/admin/grouplist.php');
		exit;
	}else{
		showMsg('编辑角色失败',config('website')."/admin/groupedit.php?id=$id");
		exit;
	}
							...
							<div class="smart-widget-body">
								<form method="post">
									<div class="form-group">
										...
										<input value="<?php echo $group['name'];?>" type="text" required name="name" class="form-control" id="name" placeholder="请输入角色名称">
									</div>
									<div class="form-group">
										<label>状态</label>
										<div>
											<div class="radio inline-block">
												<div class="custom-radio m-right-xs">
													<input type="radio" <?php echo $group['status'] ? "checked":"";?> id="show" name="status" value="1">
											...
											<div class="radio inline-block">
												<div class="custom-radio m-right-xs">
													<input type="radio" <?php echo $group['status'] ? "":"checked";?> id="hidden" name="status" value="0">
